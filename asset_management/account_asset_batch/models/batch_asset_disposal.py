...
class MoveLine(models.Model):
    ...
    asset_batch_id = fields.Many2one('batch.asset.disposal',string="Asset Batch")
...
class AccountAsset(models.Model):
    ...
    account_plus_value_id = fields.Many2one(
        comodel_name='account.account',
        string='Plus-Value Account',related='profile_id.account_plus_value_id',
        domain=[('deprecated', '=', False)],
    default=lambda self: self._default_account_plus_value_id())
    
    account_min_value_id = fields.Many2one(
        comodel_name='account.account',related='profile_id.account_min_value_id',
    ...
    def _get_posting_regime(self):
        ...
        asset = asset_obj.browse(self.id)
    ...
    def batch_remove(self):
        ...
        asset_line_vals = {
            ...
            'asset_id': asset_id,
            'name': line_name,
            'line_date': self.r_date_remove,
            'move_id': move.id,
            'type': 'remove',
        }
        asset_line_obj.create(asset_line_vals)
        asset.write({'state': 'removed', 'date_remove': self.r_date_remove})
...
class AssetBatchDisposal(models.Model):
    ...
    _rec_name = 'asset_batch_id'
    
    asset_batch_id = fields.Char("Batch ID")
    populate_by_name = fields.Char("Asset Name")
    populate_by_profile = fields.Many2one('account.asset.profile',string="Populate by Category")
    populate_by_code = fields.Char(string="Populate by Code")
    
    batch_date = fields.Date("Date",default=datetime.today())
    
    disposed_date = fields.Date("Disposed Date")
    description = fields.Text("Description")
    
    journal_line_ids = fields.One2many('account.move.line','asset_batch_id',string="Journal Entries")
    journal_ids = fields.One2many('account.move','asset_batch_id',string="Journal Entries")
    ...
    transactions = fields.One2many('account.asset','asset_batch_id',string="List of assets to be disposed")
    
    @api.model
    def create(self, values):
        """Initially, injecting sequence"""
        if values:
            values['asset_batch_id'] = self.env['ir.sequence'].next_by_code('batch.id')
        record_obj = super(AssetBatchDisposal, self).create(values)
        return record_obj
    
    @api.multi
    @api.depends('populate_by_profile','populate_by_name')
    @api.onchange('populate_by_profile','populate_by_name')
    def compute_voucher_value(self):
        lines = self.env['account.asset'].sudo().search(['|',('profile_id','=',self.populate_by_profile.id),('name','ilike',self.populate_by_name),('state','=','open')])
        #lines = self.env['account.asset'].sudo().search(['|','|',('profile_id','=',self.populate_by_profile.id),('name','ilike',self.populate_by_name),('code','=',self.populate_by_code),('state','=','open')])
        #lines = self.env['account.asset'].sudo().search(['&',['|',('profile_id','=',self.populate_by_profile),['|',('name','ilike',self.populate_by_name),('code','=',self.populate_by_code)],[('state','=','open')]]])
        self.transactions = lines
        
    def dispose_batch(self):
        #Dispose batch
        for item in self:
            errors = 0
            for asset in item['transactions']:
                if asset.state == 'open':
                    asset.batch_remove()
                if asset.state != 'removed':
                    errors += 1
        if errors == 0:
            self.invoice_status = 'journal'
            for item in self:
                for asset in item['transactions']:
                    if asset.account_move_line_ids:
                        self.journal_line_ids = self.journal_line_ids+asset.account_move_line_ids
                        for line_id in asset.account_move_line_ids:
                            if not line_id.move_id in self.journal_ids:
                                self.journal_ids = self.journal_ids + line_id.move_id
