...
class AccountAssetRemove(models.TransientModel):
    ...
    def remove(self):
        ...
        #impairment
        if asset.impairment_amount:
            move_lines_impairment = self._get_removal_data(asset, asset.impairment_amount)
            final_moves = move_lines+move_lines_impairment
            move.with_context(allow_asset=True).write({'line_ids': final_moves})
        # create move lines
