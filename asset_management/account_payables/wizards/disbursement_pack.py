...
class Invoice(models.Model):
    ...
    name = fields.Char(compute='_record_get_name')
    mobile_number = fields.Char('Contact No.')
    ...
    #wizard_id = fields.Many2one('disbursement.pack.wiz',string="Wizard ID")
    
    @api.multi
    @api.onchange('supplier_name','total_amount')
    @api.depends('supplier_name','total_amount')
    def _record_get_name(self):
        for record in self:
            record['name'] = str(record['supplier_name']) + ' R' + str(record['total_amount'])
