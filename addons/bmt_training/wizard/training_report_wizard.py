    ...
    def get_training_data(self):
        ...
        if self.report_type == 'training':
            if self.intervention_type == 'sales_pitch_bbbee':
                ...
                rep_type = 'bmt'
                rep_print_type = 'training'
                training_data = self.env['business.mgmt.training'].sudo().search([])
            elif self.intervention_type == 'coop_gov':
                type = 'Co-operative Governance Training'
                rep_type = 'all'
                rep_print_type = 'training'
                training_data = self.env['cooperative.governance.training'].sudo().search([])
        elif self.report_type == 'branch':
            training_data = []
            sp_branch_data = self.env['sales.pitch.training'].sudo().search([('branch_id', '=', self.branch_id.id)])
            bmt_data = self.env['business.mgmt.training'].sudo().search([('branch_id', '=', self.branch_id.id)])
            coop_data = self.env['cooperative.governance.training'].sudo().search([('branch_id', '=', self.branch_id.id)])
