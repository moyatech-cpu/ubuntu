...
License AGPL-3.0 or later (http://www.gnu.org/licenses/agpl).
*/
odoo.define('itm_petty_cash.budget.dashboard', function (require) {
"use strict";
...
//	template: 'itm_petty_cash.dashboard',
	events: _.extend({}, Widget.prototype.events, {
       ...
       'click .requested': 'request_money',
       'click .requested_reconcile': 'request_reconcile',
       'click .budget_expense_details_top1': 'action_budget_expense_details_top1',
       'click .budget_expense_details_top2': 'action_budget_expense_details_top2',
       'click .budget_expense_details_top3': 'action_budget_expense_details_top3',
       'click .budget_expense_details_top4': 'action_budget_expense_details_top4',
       'click .budget_expense_details_top5': 'action_budget_expense_details_top5',
       'click .current_year': 'action_current_year',
       'click .last_year': 'action_last_year',
       'click .current_year_quarter': 'action_current_year_quarter',
       'click .last_year_quarter': 'action_last_year_quarter',
	...
	init: function(parent, context) {
       ...
       var employee_data = [];
       var self = this;
       if (context.tag == 'itm_petty_cash.budget.dashboard') {
           self._rpc({
               model: 'petty.cash.dashboard',
               method: 'get_budget_info',
           }, []).then(function(result){
               self.employee_data = result[0]
           }).done(function(){
               self.render();
   ...
   render: function() {
       var super_render = this._super;
       var self = this;
       var hr_dashboard = QWeb.render( 'itm_petty_cash.budget.dashboard', {
           widget: self,
       });
       $( ".o_control_panel" ).addClass( "o_hidden" );
       $(hr_dashboard).prependTo(self.$el);
   ...
   cash_received: function(event){
       ...
       return this.do_action({
           name: _t("Budget Received"),
           type: 'ir.actions.act_window',
           res_model: 'crossovered.budget',
           view_mode: 'form',
           view_type: 'form',
           res_id: self.employee_data.budget_id,
           views: [[false, 'form']],
           context: {},
           domain: [],
           target: 'current'
   ...
   action_budget_expense_details_top1: function(event) {
   	var self = this;
       event.stopPropagation();
       event.preventDefault();
       this.do_action({
           name: _t("Budget Expenses Details"),
           type: 'ir.actions.act_window',
           res_model: 'account.payment',
           view_mode: 'tree,form',
           view_type: 'tree',
           views: [[false, 'list'],[false, 'form'],],
           domain: [['id','in', self.employee_data.budget_line_ids],['account_to_id','=',self.employee_data.budget_account_filter_top1],],
           target: 'current'
       })
   },
   action_budget_expense_details_top2: function(event) {
   	var self = this;
       event.stopPropagation();
       event.preventDefault();
       this.do_action({
           ...
           view_type: 'tree',
           views: [[false, 'list'],[false, 'form'],],
           domain: [['id','in', self.employee_data.budget_line_ids],['account_to_id','=',self.employee_data.budget_account_filter_top2],],
           target: 'current'
       })
   },
   action_budget_expense_details_top3: function(event) {
   	var self = this;
       event.stopPropagation();
       event.preventDefault();
       this.do_action({
           ...
           view_type: 'tree',
           views: [[false, 'list'],[false, 'form'],],
           domain: [['id','in', self.employee_data.budget_line_ids],['account_to_id','=',self.employee_data.budget_account_filter_top3],],
           target: 'current'
       })
   },
   action_budget_expense_details_top4: function(event) {
   	var self = this;
       event.stopPropagation();
       event.preventDefault();
       this.do_action({
           ...
           view_type: 'tree',
           views: [[false, 'list'],[false, 'form'],],
           domain: [['id','in', self.employee_data.budget_line_ids],['account_to_id','=',self.employee_data.budget_account_filter_top4],],
           target: 'current'
       })
   },
   action_budget_expense_details_top5: function(event) {
   	var self = this;
       event.stopPropagation();
       event.preventDefault();
       this.do_action({
           ...
           view_type: 'tree',
           views: [[false, 'list'],[false, 'form'],],
           views: [['id','in', self.employee_data.budget_line_ids],[false, 'list'],[false, 'graph'],],
           domain: [['account_to_id','=',self.employee_data.budget_account_filter_top5],],
           target: 'current'
       })
   },
   action_current_year: function(event){
   ...
   graph: function() {
       ...
       var myChart = new Chart(ctx, {
           ...
           data: {
               //labels: ["January","February", "March", "April", "May", "June", "July", "August", "September",
               // "October", "November", "December"],
               labels: self.employee_data.budget_expenses_labels,
               datasets: [{
                   label: 'Expenses',
                   data: self.employee_data.budget_expenses_data,
                   backgroundColor: bg_color_list,
                   borderColor: bg_color_list,
                   borderWidth: 1,
                   pointBorderColor: 'white',
           ...
           options: {
               scales: {
                   yAxes: [{
                       ticks: {
                           min: 0,
                           max: Math.max.apply(null,self.employee_data.budget_expenses_data),
                           //min: 1000,
                           //max: 100000,
                           stepSize: self.employee_data.
                           budget_expenses_data.reduce((pv,cv)=>{return pv + (parseFloat(cv)||0)},0)
                            /self.employee_data.budget_expenses_data.length
                         }
                   }]
               },
               responsive: true,
       ...
       var piectx = this.$el.find('#expenditureChart');
       bg_color_list = []
       for (var i=0;i<=self.employee_data.budget_expenses_data.length;i++){
           bg_color_list.push(self.getRandomColor())
       }
       var pieChart = new Chart(piectx, {
           type: 'pie',
...
core.action_registry.add('itm_petty_cash.budget.dashboard', PettyCashDashboardView);
return PettyCashDashboardView
});
