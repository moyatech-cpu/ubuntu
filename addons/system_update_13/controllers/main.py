...
class VoucherApplication(http.Controller):
    ...
    def grant_application(self, page=0, *args, **kwargs):
        if request.env.user.has_group('client_management.group_branch_beneficiary'):
            ...
            if existing_user:
                ...
                elif grant_app_count != len(existing_user):
                    return request.render('nyda_grant_and_voucher.check_total_app', {
    ...
    def grant_application_submit(self, **kw):
        ...
        if kw.get('supplier_quotations'):
            supplier_quotations_name = kw.get('supplier_quotations').filename
            kw['supplier_quotations'] = base64.b64encode(kw.get('supplier_quotations').read())
        else:
            supplier_quotations_name = False
            """
            fmt_data = json.loads(kw.get('form_data'))
            files = []
            for quote in fmt_data.get('supplier_quotations'):
                files.append((0, 0, quote))
            """
        ...
        if kw.get('certified_identity_document_applicant'):
            ...
            kw['certified_identity_document_applicant'] = base64.b64encode(kw.get('certified_identity_document_applicant').read())
        else:
    ...
    def voucher_application_submit(self, **kw):
        print('-------->>>>--\n\n', kw)
        #voucher_exists_id = request.env['voucher.application'].sudo().search([('client_id', '=', request.env.user.id)],limit=1, order='id desc')
        #if voucher_exists_id and (voucher_exists_id.state == 'completed' or voucher_exists_id.state == 'rejected'):
        #        return request.render('nyda_grant_and_voucher.voucher_application_exists', {
        #            'application_type': 'voucher'
        #        })
        #if voucher_exists_id and (voucher_exists_id.state != 'completed' or voucher_exists_id.state != 'decline' or voucher_exists_id.state != 'payment_completed' or voucher_exists_id.state != 'payment_released' or voucher_exists_id.state != 'approved' ):
        #        return request.render('nyda_grant_and_voucher.voucher_application_exists', {
        #            'application_type': 'voucher'
        #        })
        # existing_business and entrepreneurial_training radio button
