...
class WebsiteSupportTicket(models.Model):
    ...
    def _read_group_state(self, states, domain, order):
        ...
        #exclude_states = [customer_closed.id]
    ...
    #PR MOHAPI 03/09/2019 (Check whether this user is in the Service Desk Employee Group or Not)
    check_group = fields.Boolean(string="Check Employee Group", compute='check_employee_group')

    @api.multi
    def check_employee_group(self):
        res_user = self.env['res.users'].search([('id', '=', self._uid)])
        for record in self:
            if res_user.has_group('website_support.group_service_desk_employee') and not res_user.has_group('website_support.group_service_desk_agent'):
                record['check_group'] = True
            else:
                record['check_group'] = False
    ...
    def request_approval(self):
        ...
        return {
            ...
            'context': {'default_ticket_id': self.id, 'default_email': self.x_email_manager, 'default_subject': self.subject, 'default_approval': True, 'default_body': request_message},
            'target': 'new'
        }
    ...
    def create(self, vals):
        ...
        if new_id.category.x_sla:
            #Check if this category has a SLA response time
            category_response = self.env['website.support.sla.response'].search([('category_id','=',new_id.category.id)])
            if category_response:
                new_id.sla_id = new_id.category.x_sla.id
