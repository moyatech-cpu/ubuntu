    ...
    def base(self, data, token):
        ...
        if ks_export_boolean:
            ...
            if item._context.get('ksDateFilterSelection', False):
                ...
                if item.ks_dashboard_ninja_board_id.browse(item.ks_dashboard_ninja_board_id.id).ks_dashboard_start_date and item.ks_dashboard_ninja_board_id.browse(item.ks_dashboard_ninja_board_id.id).ks_dashboard_end_date:
                    selected_start_date = fields.datetime.strptime(item.ks_dashboard_ninja_board_id.browse(item.ks_dashboard_ninja_board_id.id).ks_dashboard_start_date,
                                                                   "%Y-%m-%d %H:%M:%S")
                    selected_end_date = fields.datetime.strptime(item.ks_dashboard_ninja_board_id.browse(item.ks_dashboard_ninja_board_id.id).ks_dashboard_end_date,
                                                                 "%Y-%m-%d %H:%M:%S")

                item = item.with_context(ksDateFilterStartDate=selected_start_date)
                item = item.with_context(ksDateFilterEndDate=selected_end_date)
                ...
                for count, index in enumerate(dataset['ks_column_type']):
                    if index == 'datetime':
                        ...
                        if dataset['data'][count]:
                            ks_converted_date = ks_convert_into_local(datetime.datetime.strptime(date_string, '%Y-%m-%d %H:%M:%S'),ks_timezone)
