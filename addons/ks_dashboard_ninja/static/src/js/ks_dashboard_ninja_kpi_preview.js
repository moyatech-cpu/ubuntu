odoo.define('ks_dashboard_ninja_list.ks_dashboard_kpi_preview', function (require) {
    ...
    var KsKpiPreview = AbstractField.extend({
        ...
        ksSum: function(count_1, count_2, item_info, field, target_1, $kpi_preview, kpi_data) {
            ...
            item_info['count'] = KsGlobalFunction._onKsGlobalFormatter(count, field.ks_data_format, field.ks_precision_digits, 2);
            item_info['count_tooltip'] = field_utils.format.float(count, Float64Array, {digits: [0, 2]});
                    ...
                    case "Ratio":
                       var gcd = self.ks_get_gcd(Math.round(count_1), Math.round(count_2));
                        if (field.ks_data_format == 'exact'){
                            ...
                            item_info['count'] = field_utils.format.float(count_1 / gcd, Float64Array,{digits: [0, 2]}) + ":" + field_utils.format.float(count_2 / gcd, Float64Array, {digits: [0, 2]});
                            } else {
